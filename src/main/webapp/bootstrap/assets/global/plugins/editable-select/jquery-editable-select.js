...
+(function ($) {
	...
	EditableSelect = function (select, options) {
		...
		if (['focus', 'manual','click'].indexOf(this.options.trigger) < 0) this.options.trigger = 'click';
	...
	EditableSelect.DEFAULTS = { filter: true, effects: 'default', duration: 'fast', trigger: 'click' };
	...
	EditableSelect.prototype.show = function (e) {
		if(e != undefined){
	...
	EditableSelect.prototype.removeAll = function () {
	...
	EditableSelectUtility.prototype.initializeInput = function () {
		...
		switch (this.es.options.trigger) {
			...
			case 'click':
				that.es.$input
					.on('click', $.proxy(that.es.show, that.es));
				$(document)
					.on('click', $.proxy(that.es.hide, that.es));
				break;
			case 'focus':
				that.es.$input
