	...
	if(!grid.isInit()){
		...
		grid.init({
			...
			dataTable : {
				...
				"ajax" : {
					"url":server+"/teacher/testinstancerequestusers/selectTestInstanceRequestUsersList.do", // ajax source
				},
...
$("#addBtn").on("click", function(){
	var testInstanceRequestId = $("#testInstanceRequestId").val();
	var url = server+"/pages/teacher/vrnewexam/newexamAuthoruserEdit_dlg.jsp";
	var options = {
			...
			clear:"true",
			init:function(){
				initEdit(testInstanceRequestId);
			},
			css:{"width":"large"},
			callback:function(){
			}
	...
	if(rows.length == 0){
		...
		utils.dialog.confirm("lms.deleteConfirmMsg", function(data){
			if(data){
				...
				var testInstanceRequestId = $("#testInstanceRequestId").val();
				var updateData = {"idList":params, "testInstanceRequestId":testInstanceRequestId};
				var url = server + "/teacher/testinstancerequestusers/deleteTestInstanceRequestUsers.do";
