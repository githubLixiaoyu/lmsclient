...
function initEdit(testInstanceRequestId){
//	$("#portlet-upload-dlg .col-md-12").css("height", $(window).height()*0.7);
	$("#testInstanceRequestId").val(testInstanceRequestId);
	//所属部门
	...
	if(!userGrid.isInit()){
		...
		userGrid.init({
			...
			dataTable : {
				...
				"ajax" : {
					"url":server+"/teacher/testinstancerequestusers/selectCanSelectForTestInstanceRequestList.do", // ajax source
				},
...
$("#selectBtn").on("click", function(){
	...
	if(idList.length == 0){
		...
		var updateData = {"idList":idList, "testInstanceRequestId":testInstanceRequestId};
		var url = server + "/teacher/testinstancerequestusers/saveTestInstanceRequestUsers.do";
