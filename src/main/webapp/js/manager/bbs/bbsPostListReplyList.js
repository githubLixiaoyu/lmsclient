	...
	if(!grid.isInit()){
		...
		grid.init({
			...
			dataTable : {
				...
				"ajax" : {
					"url":server+"/manager/tbbspostreply/selectBbsPostReplyList.do", // ajax source
				},
	...
	if(rows.length == 0){
		...
		utils.dialog.confirm("lms.deleteConfirmMsg", function(data){
			if(data){
				...
				var url = server + "/manager/tbbspostreply/deleteBbsPostReply.do";
...
//	window.history.go(-1);
	window.location.href= server+"/pages/manager/bbs/bbsPostList.jsp";
});
