package com.zxtech.support.i18n; 
...
import com.zxtech.support.i18n.service.ResourceBundleI18nServiceImpl;
import com.zxtech.support.i18n.support.I18nServiceAccessor;
...
//			}
	...
	public static String getMessage(String key, Object[] args,String locale) {
		...
		if ("".equals(locale) || locale == null) {
			String message = null;
			try {
				message = accessor.getMessage(key, args,Locale.getDefault());
	...
	public static Map<String, Object> getAllMessage(String locale) {
		initResourceBundleI18nServiceImpl();
		resourceBundleI18nServiceImpl.setBasenames(basename.toArray(new String[basename.size()]));
		I18nServiceAccessor accessor = resourceBundleI18nServiceImpl.getAccessor();
		// 如果没有指定locale属性，就用默认的Locale
		if ("".equals(locale) || locale == null) {
			...
			try {
				message = accessor.getAllMessage(Locale.getDefault());
			} catch (NoSuchMessageException e) {
			}
			return message;
		}
//		return accessor.getAllMessage(new Locale(locale));
		Locale l = null;
		if ("zh".equals(locale.substring(0, 2))) {
			l = Locale.SIMPLIFIED_CHINESE;
		} else {
			l = Locale.US;
		}
		
		return accessor.getAllMessage(l);
	}
	
	public static Map<String, Object> getAllMessage() {
		return getAllMessage(null);
	}
