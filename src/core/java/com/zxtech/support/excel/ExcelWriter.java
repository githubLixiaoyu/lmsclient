...
public class ExcelWriter {
	...
	public ExcelWriter(OutputStream out, Version version) {
		excelTemplate = new ExcelWriterTemplate(out,version);
	}
	 ...
	 * @param data
	 */
	public void write(final int sheetIndex, final int rowFrom, final int colFrom, final int headRows,final List<Object[]> data) {
		excelTemplate.execute(new ExcelWriterCallback() {
			...
			public void doInPoi(Workbook workbook) {
				...
				write(sheet, rowFrom, colFrom, headRows, data);
			}
		});
	}
	 ...
	 * @param data
	 */
	public<T> void write(final String sheetName, final int rowFrom, final int colFrom, final int headRows,final List<T> data) {
		excelTemplate.execute(new ExcelWriterCallback() {
	...
	private<T> void write(Sheet sheet, int rowFrom, int colFrom, List<T> data) {
		...
		List<CellStyle> cellStyleList = new ArrayList<CellStyle>();//保存列样式
		for (T dataRow : data) {
			Row row = sheet.createRow(rowIndex);
			...
			if(dataRow instanceof Object[]){//不带样式的纯数据
				...
				for (ExcelCellInfo dataCellInfo : dataRowInfo.getCells()) {
					...
					if(rowIndex == rowFrom){
						...
						cellStyle = cellStyleList.get(colIndex-colFrom);
					}
	...
	private<T> void write(Sheet sheet, int rowFrom, int colFrom,int headRows, List<T> data) {
		int rowIndex = rowFrom;
	...
	private void setCellValue(Cell cell, Object data) {
		...
		if (data instanceof Date) {
			cell.setCellValue(DateFormatUtils.format((Date) data, DateFormatUtils.ISO_DATE_FORMAT.getPattern()));
